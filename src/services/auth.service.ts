...
export class AuthService {
  ...
  async deleteUser(username: string) {
    try {
      ...
      return { user, status: new Constants().PREMADE_STATUS.SUCCESS_DELETED };
    } catch (error) {
      return {
  ...
  async verify(username: string, code: string) {
    ...
    const response = await this.smsService.checkVerification(username, code);
    if (response === 'approved') {
      await this.smsService.sendSMS(
        username,
        'Hello, Welcome to My Clinic App',
      );
      return {
        message: 'Approved!',
        status: 1,
  ...
  async register(credentials: RegistrationDTO) {
    try {
      ...
      return { user: { ...user.toJSON(), token }, status: new Constants().PREMADE_STATUS.Success_Created };
    } catch (error) {
      if (error.code === '23505') {
        throw new ConflictException('Username has already been taken');
      }
      return {
        status: new Constants().PREMADE_STATUS.Fail_Alone,
        error,
      };
    }
  }
  ...
  async login(credentials: LoginDTO) {
    try {
      ...
      const payload = { username: user.username };
      const token = this.jwtService.sign(payload);
      return { user: { ...user.toJSON(), token }, status: new Constants().PREMADE_STATUS.Success_Alone };
    } catch (error) {
  ...
  async registerDoctor(credentials: RegistrationDTO) {
    try {
      ...
      await this.smsService.sendVerification(user.username);
      const token = this.jwtService.sign(payload);
      return { doctor: { ...user.toJSON(), token }, status: new Constants().PREMADE_STATUS.Success_Created };
    } catch (error) {
      if (error.code === '23505') {
        throw new ConflictException('Username has already been taken');
      }
  ...
  async loginDoctor(credentials: LoginDTO) {
    try {
      ...
      const payload = { username: user.username };
      const token = this.jwtService.sign(payload);
      return { doctor: { ...user.toJSON(), token }, status: new Constants().PREMADE_STATUS.Success_Alone, };
    } catch (error) {
      throw new UnauthorizedException('Invalid Credintials');
    }
  }
  ...
  async refreshToken(username: string) {
    try {
      ...
      if (username === undefined) {
        const error = new Error('Please provide a good username');
        return {
          error,
          status: 0,
        };
      ...
      return {
        token,
        status: new Constants().PREMADE_STATUS.Success_Alone
      };
    } catch (error) {
      return {
        status: new Constants().PREMADE_STATUS.Fail_GET,
